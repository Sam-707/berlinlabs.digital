...
const OrderEntryView: React.FC<OrderEntryViewProps> = ({ tableNumber, onConfirm, onBack, activeOrder, menu }) => {
  ...
  return (
    <div className="flex flex-col h-full bg-[#170e10] text-white animate-fade-in overflow-hidden relative">
      <header className="px-6 pt-12 pb-6 flex items-center justify-between border-b border-white/5 bg-[#170e10]/90 backdrop-blur-xl z-30">
      ...
      <main className="flex-1 flex flex-col p-6 pt-10 overflow-hidden">
        {/* Match Card */}
        <div className="bg-gradient-to-br from-primary/20 to-primary/5 rounded-[2.5rem] p-8 border border-primary/20 shadow-2xl relative overflow-hidden mb-8 text-center animate-slide-up">
           <div className="flex items-center justify-center gap-6 mb-6">
             <div className="flex flex-col items-center">
                <span className="text-[9px] font-black text-primary uppercase tracking-widest mb-1">Code</span>
                <span className="text-3xl font-black text-white">{activeOrder.id}</span>
             </div>
             <span className="material-symbols-outlined text-primary/50 text-[32px]">sync_alt</span>
             <div className="flex flex-col items-center">
                <span className="text-[9px] font-black text-primary uppercase tracking-widest mb-1">Table</span>
        ...
        <div className="flex-1 flex flex-col bg-[#241619] rounded-[2.5rem] p-6 border border-white/10 shadow-3xl overflow-hidden">
          <div className="flex justify-between items-start mb-6">
            <h2 className="text-xl font-black tracking-tight">Order Details</h2>
            <div className="bg-primary/10 px-3 py-1 rounded-full text-primary text-[10px] font-black uppercase tracking-widest">
              ...
              return (
                <div key={cartItem.id} className="flex justify-between items-center py-3 border-b border-white/5 last:border-0">
                  <div className="flex gap-4 items-center">
                    <span className="flex size-7 items-center justify-center rounded-lg bg-primary/20 text-primary font-black text-xs">
                      {cartItem.quantity}
                    </span>
                    <span className="font-bold text-sm text-white/90">{item?.name}</span>
                  </div>
                  <span className="text-[11px] font-black text-text-secondary/40 tracking-wider">
