...
const InventoryView: React.FC<InventoryViewProps> = ({ menu, setMenu, onBack, onImport }) => {
  ...
  const toggleAvailability = async (id: string) => {
    ...
    const originalAvailability = itemToToggle.isAvailable;
    ...
    try {
      ...
      // Rollback on failure
      const rolledBack = menu.map(item =>
        item.id === id ? { ...item, isAvailable: originalAvailability } : item
      );
      setMenu(rolledBack);
      triggerErrorToast('Failed to update. Please try again.');
      console.error('[InventoryView] Failed to toggle availability:', error);
    }
  };
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file && editingItem) {
      try {
        ...
        triggerErrorToast('Failed to upload image. Please try again.');
        console.error('[InventoryView] Failed to upload image:', error);
      }
    }
  };
  ...
  const saveItem = async () => {
    ...
    if (!editingItem.name?.trim()) {
      triggerErrorToast('Name is required');
      return;
    }
    if (!editingItem.category?.trim()) {
      triggerErrorToast('Category is required');
      return;
    }

    // Validate price (handle string or number)
    const parsedPrice = typeof editingItem.price === 'string'
      ? parseFloat(editingItem.price)
      : editingItem.price;

    if (editingItem.price === '' || isNaN(parsedPrice) || parsedPrice <= 0) {
      triggerErrorToast('Price must be a valid number greater than 0');
      return;
    }
    ...
    const exists = menu.find(i => i.id === trimmedItem.id);
    let updatedMenu: MenuItem[];

    if (exists) {
      updatedMenu = menu.map(i => i.id === trimmedItem.id ? (trimmedItem as MenuItem) : i);
    } else {
      updatedMenu = [...menu, trimmedItem as MenuItem];
    }
    ...
    try {
      ...
      // Rollback on failure
      const rolledBack = menu.map(i =>
        i.id === trimmedItem.id ? { ...i, isAvailable: i.isAvailable } : i
      );
      setMenu(rolledBack);
      triggerErrorToast('Failed to save item. Please try again.');
      console.error('[InventoryView] Failed to save item:', error);
    }
  };
  ...
  return (
    <div className="flex flex-col h-full bg-background-dark text-white animate-fade-in overflow-hidden relative">
      <header className="px-6 pt-12 pb-6 bg-background-dark/90 backdrop-blur-xl border-b border-white/5 sticky top-0 z-30">
      ...
      <main className="flex-1 overflow-y-auto p-6 space-y-6 pb-36 no-scrollbar">
        <div className="flex items-center justify-between px-1">
          ...
          <span className="text-[10px] font-black text-white uppercase tracking-widest">{filteredItems.length} Live Items</span>
        </div>
        
        <div className="space-y-4">
          {filteredItems.map(item => (
            <div key={item.id} className={`group bg-surface-dark rounded-[1.75rem] p-5 border border-white/5 shadow-2xl transition-all ${!item.isAvailable ? 'grayscale opacity-60 scale-[0.98]' : 'hover:scale-[1.01]'}`}>
              <div className="flex gap-5">
                ...
                <div className="flex-1 min-w-0 flex flex-col justify-between py-1">
                  ...
                  <div className="mt-4 flex items-center justify-between">
                    <div className="text-lg font-black text-price tracking-tight">€{item.price.toFixed(2)}</div>
      ...
      {showErrorToast && (
        <div className="fixed top-32 left-1/2 -translate-x-1/2 z-[110] animate-bounce">
           <div className="bg-red-500 text-white px-4 py-2 rounded-full text-[10px] font-black uppercase tracking-widest shadow-xl flex items-center gap-2">
               ...
               <div className="flex items-center gap-6">
                  <div className="relative group">
                    <div className="size-24 rounded-2xl border-2 border-white/5 overflow-hidden bg-white/5">
                       <img src={editingItem.image} alt={editingItem.name || 'Menu item preview'} className="w-full h-full object-cover" />
                    </div>
                    <button 
                  ...
                  <div className="flex-1 space-y-4">
                     ...
                     <div>
                        ...
                        <input
                          ...
                          onChange={e => setEditingItem({...editingItem, price: e.target.value === '' ? '' : (parseFloat(e.target.value) || 0)})}
                        />
                     </div>
                  </div>
               </div>
